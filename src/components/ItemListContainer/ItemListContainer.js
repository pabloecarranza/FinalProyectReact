...
const ItemListContainer = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const getData = async () => {
      const dataFS = await getDocs(collection(db, "items")); //items
      ...
      setData(
        productsFS.filter((item) =>
          params.id === "all" || activeCategory === "all"
            ? productsFS
      ...
      <div className="category-container">
        <ButtonGroup variant="outlined" size={buttonStyle}>
          <Link
            ...
            to={`/category/all`}
          >
            <Button color={params.id === "all" ? "secondary" : "primary"}>
              all
            </Button>
          </Link>
          ...
          {categories.map((category) => {
            return (
              ...
              >
                <Button
                  color={
                    activeCategory === category && params.id !== "all"
                      ? "secondary"
                      : "primary"
                  }
                >
